...
public class UserLoginController extends BaseController {
    ...
    public Map<String, Object> Login(ModelAndView modelAndView) throws IOException {
        ...
        for (User user : users) {
            ...
            if (username.equals(user.getUserId()) && password.equals(user.getPassword())) {
                ...
                if (!staffInfoService.getStaffByDynamic("", "", "", username, null, null, null, null)
                        .isEmpty()) {
                    session.setAttribute("userId", staffInfoService
                ...
                if (userRoles.equals("客户")) {
                    List<Contract> contract = customerManageService.selectByCust(user.getCustName(), null, null, null, null,
    ...
    public Map<String, Object> client(ModelAndView modelAndView) throws Exception {
        ...
        // 客户只能看到自己的合同
        List<Contract> contracts = customerManageService.selectByCust(custName, "", "", "", null, null, null, null, null, null);
        // 获取当月所有工单
        ...
        // 总设备数
        List<Device> devices = customerManageService.selectByDevice(custName, "", "", "", null, null);
        // 客户资产数
        List<Device> custDevices = customerManageService.selectByDevice(custName, "", "", "客户", null, null);
        int serviceDevices = 0;
        if (devices != null && custDevices != null) {
            serviceDevices = devices.size() - custDevices.size()
                    - customerManageService.selectByDevice(custName, "", "", "客户", null, null).size();
        }
        // 未完成
        int unFinished = 0;
        // 超时工单
    ...
    public Map<String, Object> majordomo() throws Exception {
        ...
        Map<String, Object> json = new HashMap<>();
        // 运营总监可以看到所有客户的合同
        List<Contract> contracts = customerManageService.selectByComp("");
        // 查找已到期合同
        List<Contract> timeContracts = customerManageService.selectByCust("", "", "1", "", null, null, null, null, null, null);
        // 查找一年内到期合同
        List<Contract> dueToContracts = customerManageService.selectByCust("", "", "", "1", null, null, null, null, null, null);
        // 运营总监可以看到所有工单
        Calendar cale = null;
        cale = Calendar.getInstance();
        int year = cale.get(Calendar.YEAR);
    ...
    public Map<String, Object> operations(ModelAndView modelAndView) throws Exception {
        ...
        List<Contract> contracts = customerManageService.selectByComp(custName);
        // 查找到期合同
        List<Contract> timeContracts = customerManageService.selectByCust("", custName, "1", "", null, null, null, null, null, null);
        // 查找一年内到期合同
        List<Contract> dueToContracts = customerManageService.selectByCust("", custName, "", "1", null, null, null, null,
                null, null);
        // ****************************************************************
    ...
    public ModelAndView getOpenId(ModelAndView modelAndView) throws IOException {
        ...
        String custName = customerManageService.selectByCode(machCode).getCustArea(); // 所属客户名称
        ...
        for (User user : userService.selectAllUser()) {
            ...
            if (user.getOpenId().equals(openid)) {
                 ...
                 * serviceManageService.selectOrderByDynamic(null, machCode,
